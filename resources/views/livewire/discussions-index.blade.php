<div>
    <div class="flex flex-col space-y-3 filters md:flex-row md:space-y-0 md:space-x-6">
        ...
        <div class="w-full md:w-1/3">
            <select wire:model="filter" name="other_filters" id="other_filters" class="w-full px-4 py-2 border-none rounded-xl">
                ...
                @admin
                    <option value="Spam Discussion">Spam Discussion</option>
    ...
    <div class="my-8 space-y-6 ideas-container">
        @forelse ($discussion as $discuss)
            <livewire:discussion-index :key="$discuss->id" :discuss="$discuss" :votesCount="$discuss->votes_count" />
        @empty
            <div class="mx-auto mt-12 w-70">
                <img src="{{ asset('img/no-ideas.svg') }}" alt="No Result!" class="mx-auto mix-blend-luminosity">
                <div class="mt-6 font-bold text-center text-gray-400">No result were found...</div> 
            </div>
        @endforelse
