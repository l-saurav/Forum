<div x-data="{ isOpen: false }"
    ...
    Livewire.hook('message.processed', (message, component) => {
        ...
        {{--Adding Comment--}}
        if((message.updateQueue[0].payload.event === 'commentWasAdded' || message.updateQueue[0].payload.event === 'statusWasUpdated')&& message.component.fingerprint.name === 'discussion-comments'){
        ...
        @else
            <div class="px-4 py-6">
                ...
                <div class="flex items-center mt-8 space-x-3">
                    <a wire:click.prevent="redirectToLogin" href="{{ route('login') }}" class="w-1/2 px-6 py-3 text-sm font-semibold text-center text-white transition duration-150 ease-in h-11 bg-blue rounded-xl hover:bg-blue-hover">Login</a>
                    <a wire:click.prevent="redirectToRegister" href="{{ route('register') }}" class="flex items-center justify-center w-1/2 px-6 py-3 text-xs font-semibold transition duration-150 ease-in bg-gray-200 border border-gray-200 h-11 rounded-xl hover:border-gray-400">Sign Up</a>
                </div>
            </div>
        @endauth
    </div>
