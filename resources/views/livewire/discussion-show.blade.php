<div class="container idea-and-buttons">
        <div class="flex mt-4 bg-white idea-container rounded-xl">
            <div class="flex flex-col flex-1 px-4 py-6 md:flex-row">
                <div class="flex-none mx-2">
                    <a href="#">
                        <img src="{{ $discussion->user->getAvatar() }}" alt="avatar" class="w-14 h-14 rounded-xl">
                    </a>
                </div>
                <div class="w-full mx-2 md:mx-4">
                    ...
                    <div class="mt-3 text-gray-600">
                        @admin
                            @if ($discussion->spam_reports > 0 )
                                <div class="mb-2 text-red">Spam Reports: {{$discussion->spam_reports}}</div>
                            @endif
                        @endadmin
                        {!! nl2br(e($discussion->description)) !!}
                    </div>

                    <div class="flex flex-col justify-between mt-6 md:flex-row md:items-center">
                        <div class="flex items-center space-x-2 text-xs font-semibold text-gray-400">
                            <div class="hidden font-bold text-gray-900 md:block">{{ $discussion->user->name }}</div>
                            <div class="hidden md:block">&bull;</div>
                            <div>{{ $discussion->created_at->diffForHumans() }}</div>
                            <div>&bull;</div>
                            <div>{{$discussion->category->name}}</div>
                            <div>&bull;</div>
                            <div class="text-gray-900">{{$discussion->comments()->count()}} Comments</div>
                        </div>
                        <div
                            class="flex items-center mt-4 space-x-2 md:mt-0"
                            x-data="{ isOpen: false }">
                            <div class="{{'status-'.Str::kebab($discussion->status->name)}} text-xxs font-bold uppercase leading-none rounded-full text-center w-28 h-7 py-2 px-4">{{ $discussion->status->name }}</div>
                            @auth
                                <div class="relative">
                                    <button
                                    ...
                                    <ul
                                        ...
                                        <li><a href="#" @click.prevent="isOpen=false
                                            $dispatch('custom-show-edit-modal')" class="block px-5 py-3 transition duration-150 ease-in hover:bg-gray-100">Edit Discussion</a></li>
                                        @endcan
                                        @can('delete',$discussion)
                                        <li><a href="#" @click.prevent="isOpen=false
                                            $dispatch('custom-show-delete-modal')" class="block px-5 py-3 transition duration-150 ease-in hover:bg-gray-100">Delete Discussion</a></li>
                                        @endcan
                                        <li><a href="#"  @click.prevent="isOpen=false
                                        $dispatch('custom-show-mark-discussion-as-spam-modal')" class="block px-5 py-3 transition duration-150 ease-in hover:bg-gray-100">Mark as Spam</a></li>
                                        @admin
                                            @if ($discussion->spam_reports > 0)
                                            <li><a href="#"  @click.prevent="isOpen=false
                                                $dispatch('custom-show-mark-discussion-as-not-spam-modal')" class="block px-5 py-3 transition duration-150 ease-in hover:bg-gray-100">Not a Spam</a></li>
                                            @endif
                                        @endadmin
                                    </ul>
                                </div>
        ...
        <div class="flex items-center justify-between mt-6 buttons-container">
            <div class="flex flex-col items-center space-x-4 md:flex-row md:ml-6">
                <livewire:add-comment :discussion="$discussion" />
                @admin
                    <livewire:set-status :discussion="$discussion" />
                @endadmin
            </div>
