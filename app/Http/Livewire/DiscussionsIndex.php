...
class DiscussionsIndex extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.discussions-index', [
            'discussion' => discussion::with('user','category','status')
            ...
            })->when($this->filter && $this->filter === 'Spam Discussion', function ($query) {
            ...
            ->addSelect(['voted_by_user' => Vote::select('id')->where('user_id',auth()->id())->whereColumn('discussion_id','discussions.id')])->withCount('votes')->withCount('comments')->orderBy('id','desc')->simplePaginate()->withQueryString(),
